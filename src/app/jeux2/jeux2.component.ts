...
@Component({
  selector: 'app-jeux2',
  templateUrl: './jeux2.component.html',
  styleUrls: ['./jeux2.component.css']
})
export class Jeux2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const ucanvas: any = get('upcoming');
    const uctx: any = ucanvas.getContext('2d');
    ...
    let dx = 0; // pixel size of a single tetris block
    let dy = 0; // pixel size of a single tetris block
    ...
    let playing = false;       // true|false - game is in progress
    let dt = 0;            // time since starting this game
    let current: any = null;       // the current piece
    let next: any = null;          // the next piece
    let score = 0;        // the current score
    let vscore = 0;        // the currently displayed score (it catches up to score in small chunks - like a spinning slot machine)
    let rows = 0;         // number of completed rows in the current game
    ...
    function eachblock(type, x: number, y: number, dir: any, fn: any) {
      let bit: any;
      let row = 0;
      let col = 0;
      let tmpBlocks: any = type.blocks[dir];
    ...
    function setNextPiece(piece = null) { next = piece || randomPiece(); invalidateNext(); }
    ...
    function drawCourt() {
      if (invalid.court) {
        ...
        for (let y = 0; y < ny; y++) {
          for (let x = 0; x < nx; x++) {
            if (blockTmp = getBlock(x, y))
              drawBlock(ctx, x, y, blockTmp.color);
          }
        }
